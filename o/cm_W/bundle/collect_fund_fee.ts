...
// instruction collect_fund_fee: handler + 26 reachable functions
...
// instruction handler: collect_fund_fee (discriminator sha256("global:collect_fund_fee")[..8] = 0x7e06c2df954e8aa7)
...
function ix_collect_fund_fee(a: u64, program_id: u64, accounts: u64, accounts_len: u64, ix_args: u64, ix_args_len: u64): u64 {
	...
	let l, p: u64
	const i = sol_log("Instruction: CollectFundFee", 0x1b)
	const f = ix_args_len
	if (f >= 8 && (f & -8) != 8) {
		const args: CollectFundFeeArgs = ix_args
		...
		p = accounts_collect_fund_fee(sc8, amount_0_requested, s1a0, undef, fp, i)
		const k = ld64(sb8)
		l = ld64(sc8 + 8)
		const j = ld64(sc8)
		if (j == 0) {
		...
		p = fn_4e170(s1b0, sc8, amount_0_requested, amount_1_requested, undef, m)
		l = ld64(s1b0)
		if (l == 2) {
...
// Anchor Accounts::try_accounts of instruction collect_fund_fee (called by ix_collect_fund_fee; name [str]: from the handler's "Instruction: …" log; was fn_c7a98)
		...
		if (f == 2) {
			...
			if ((t & -8) != 0) {
				...
				if ((ac & -8) != 0) {
					...
					const ba = ld64(az)
					copyr(s1d0, ba, 0x20)
					const bb = memcmp(s1d0, s280, 0x20)
					let bc = bb as u32
					if (bc != 0) {
						copyr(sd8, ba, 0x20)
...
// types [heur]: b: CollectFundFeeContext (the handler ix_collect_fund_fee passes a frame object holding (program_id, address of a copy of the Accounts result))
function fn_4e170(a: u64, b: CollectFundFeeContext, c: u64, d: u64, e: u64, r0: u64): u64 {
	...
	let bz: u64
	const accounts: CollectFundFeeAccounts = b.accounts
	...
	const h = ld64(g + 0x428)
	st64(g + 0x428, h - min(h, d))
	const i = ld64(g + 0x420)
	st64(g + 0x420, i - min(i, c))
	st64(j, ld64(j) + 1)
