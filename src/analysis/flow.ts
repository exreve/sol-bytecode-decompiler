...
import { shortName } from './facts.ts'
...
export interface ExitFn { type?: string; param: number; fields: { off: number; size: number; name: string }[]; subs?: { type?: string; fields: ExitFn['fields']; name?: string }[] } // subs: a wrapper's several objects (an Accounts struct's exit), each named by the check after its exit call
...
export function exitFns(r: Result): Map<number, ExitFn> {
	...
	for (const fo of r.funcs) {
		...
		let sdefs: Map<number, Expr> | undefined
		...
		for (const s of stmtsOf(fo)) {
			...
			// (through a variable defined once)
			if (fo2 !== undefined) { let v = slot.get(fo2); if (v?.k === 'var') v = (sdefs ??= singleDefs(fo)).get(v.id); if (v?.k === 'load' && v.size === len) src = v.addr }
		...
		for (const rec of byW.values()) {
			const ws = rec.writes
			// (one field write: only for a type the IDL names by its discriminator)
			if (!ws.length || (ws.length < 2 && !(idl && rec.disc !== undefined && discType.has(rec.disc))) || rec.disc === undefined || !ws.every(w => w.param === ws[0].param)) continue
	...
	for (let round = 0; round < 2; round++) for (const fo of r.funcs) {
		...
		const subs: { pi: number; ex: ExitFn; d: number; pc: number }[] = []
		for (const s of stmtsOf(fo)) {
			const c = callOf(s)
			...
			const pi = params.get(base)
			if (!ex || pi === undefined || (subs.length && pi !== subs[0].pi)) continue
			subs.push({ pi, ex, d: offOf(a!, base)!, pc: s.pc })
		}
		if (!subs.length) continue
		const [{ pi, ex, d }] = subs
		const shift = (e: ExitFn, d: number) => e.fields.map(x => ({ ...x, off: x.off + d }))
		// (each object named by the check right after its exit call, e.g. the error with the account's name)
		const ff = r.facts.get(fo.pc)
		// (else the account name its error path builds (heap string) before the next exit call)
		const lines = subs.map(x => ff?.pcLine.get(x.pc))
		const nameAt = (pc: number) => {
			const i = subs.findIndex(x => x.pc === pc), l = lines[i]
			if (l === undefined) return undefined
			const k = ff!.checks.filter(k => k.named && k.line > l && (lines[i + 1] === undefined || k.line < lines[i + 1]!)).sort((x, y) => x.line - y.line)[0]?.named
			if (k) return k
			const t = ff!.lines.slice(l, lines[i + 1] ?? ff!.lines.length).join('\n')
			return /\/\/ "([a-z][a-z0-9_]*)"/.exec(t)?.[1] ?? shortName(t)
		}
		out.set(fo.pc, { type: ex.type, param: pi, fields: shift(ex, d), subs: subs.length > 1 ? subs.map(x => ({ type: x.ex.type, fields: shift(x.ex, x.d), name: nameAt(x.pc) })) : undefined })
	}
	return out
}
...
export function addExitWrites(r: Result) {
	...
	for (const fo of r.funcs) {
		...
		for (const s0 of sites) for (const ex of exitObjs(exits.get((callOf(s0)!.t as { pc: number }).pc)!)) {
			...
			const acct = ex.name ?? ff.checks.find(k => k.before === callee && k.named)?.named ?? (ex.type ? ex.type.replace(/([a-z0-9])([A-Z])/g, '$1_$2').toLowerCase() : 'account?')
...
function anchorEval0(r: Result, H: FuncOut, objs: FrameObj[], exits: Map<number, ExitFn>): AnchorEval {
	...
	const accountsParam = paramVar(H.f, 3)
	/** the handler's frame word z is the pointer of the accounts slice (the handler's accounts parameter) the try call gets */
	const restSlice = (z: number, c: Extract<Expr, { k: 'call' }>, p: number): boolean => {
		if (!c.args.some(x => D.fpOff(x) === z)) return false
		const y = D.reaching(D.SLOT(z), p)
		...
		const ev0 = (e: Expr, p: number, d: number): HVal | undefined => {
			...
			switch (e.k) {
				...
				case 'bin': {
					...
					if (a?.k === 'info' && a.seq && a.off === 0 && c > 0 && c % 0x30 === 0) { const q = a.seq.slice(c / 0x30); return q.length ? { k: 'info', acct: q[0], seq: q, off: 0 } : undefined }
...
export function tryInfo(r: Result, H: FuncOut): TryInfo | undefined {
	...
	const known = r.tryOf.get(H.pc), lay = r.acctLayouts?.get(H.pc) ?? []
	{
		...
		if (T && tf) {
			...
			// (the accounts slice try_accounts takes its accounts from: &mut &[AccountInfo], its third parameter)
			const slicePv = T.f.vars.find(v => v.param === 3)?.id
			// (a variable holding a copy of it (its other definitions: registers a call clobbered))
			const copies = (id: number) => { let any = false, other = false; for (const b of T.f.blocks) for (const s of b.stmts) if ((s.k === 'set' || s.k === 'call') && s.dst === id) { if (s.k === 'set' && s.e.k === 'var' && s.e.id === slicePv) any = true; else if (!(s.k === 'set' && s.e.k === 'undef')) other = true } return any && !other }
			...
			T.f.blocks.forEach((b, bi) => { const bl: Field[] = [], wordOf = new Map<number, number | undefined>(); let nst = 0; b.stmts.forEach((s, i) => {
				...
				if (!y || y[0].k !== 'call' || y[0].t.k !== 'fn') {
					...
					let vn = s.v.k === 'var' ? T.names[s.v.id] : undefined
					// (else the account whose missing-account check (AccountNotEnoughKeys) is the nearest one on the way to the load of
					// the slice's pointer; a variable assigned on several paths (the error path's value too): its definition loading it)
					const fromSlice = (e: Expr, q: number) => { if (e.k !== 'load' || e.size !== 8) return false; const isS = (x: Expr) => x.k === 'var' && (x.id === slicePv || copies(x.id)); const z = follow(e.addr, q); return isS(e.addr) || (!!z && isS(z[0])) }
					let lp: number | undefined
					if (y && fromSlice(y[0], y[1])) lp = y[1]
					else if (y && y[0].k === 'var') { const id = y[0].id; T.f.blocks.forEach((b2, bi2) => b2.stmts.forEach((s2, i2) => { if (lp === undefined && s2.k === 'set' && s2.dst === id && fromSlice(s2.e, bi2 << 16 | i2)) lp = bi2 << 16 | i2 })) }
				...
				// (offset 0: the Result's tag, unless an account's (the niche of its non-null pointer, see above))
				if (off === 0) return
				const [e, p] = y
	...
	else if (res) { const se = sliceEvents(r, H, res.tryPc); res.ptrs = se?.ptrs; res.seqs = se?.seqs }
 ...
 */
/**
 * The accounts try_accounts consumes from its accounts slice (its third parameter), in the IDL's order: a call taking
 * the slice (the account's own try_accounts: its out object) or a load of the slice's pointer (an &AccountInfo taken
 * as is), one each, on a chain of blocks each dominating the next. tpc: try_accounts (else the call the handler passes
 * its accounts slice to: a frame {ptr, len} holding its accounts parameter).
 */
function sliceEvents(r: Result, H: FuncOut, tpc?: number) {
	...
	if (tpc === undefined && ap !== undefined) H.f.blocks.forEach((b, bi) => b.stmts.forEach((s, i) => {
		const c = callOf(s)
	...
	const TD = defsOf(T.f, cl), g = cfgOf(T), sv = paramVar(T.f, 3)
	/** the slice parameter, through copies and a frame word it was saved to */
	const isS = (e: Expr, d = 0): boolean => {
		if (e.k !== 'var' || d > 4) return false
		if (e.id === sv) return true
		const x = TD.defs.get(e.id)
		if (!x) return false
		if (x.k === 'var') return isS(x, d + 1)
		const z = x.k === 'load' && x.size === 8 ? TD.fpOff(x.addr) : undefined, y = z === undefined ? null : TD.reaching(TD.SLOT(z), TD.defPos.get(e.id)!)
		return !!y && isS(y[0], d + 1)
	}
	// (consumptions: a call (its out object), a load of the pointer; advances: the pointer stored back past a loaded
	// one (an account taken at an offset from it, e.g. `g + 0x30` for the next one))
	const all: { b: number; i: number; out?: number; v?: number; adv?: [number, number]; idx: number }[] = []
	// (not a call on a failing side (an error built))
	const errPc = new Set((r.facts.get(T.pc)?.calls ?? []).filter(c => c.errPath && c.pc !== undefined).map(c => c.pc!))
	T.f.blocks.forEach((b, bi) => b.stmts.forEach((s, i) => {
		if (g.rpo[bi] < 0) return
		const c = callOf(s)
		if (c && c.t.k === 'fn' && c.args.some(isS) && !errPc.has(s.pc)) all.push({ b: bi, i, out: TD.fpOff(c.args[0]), idx: -1 })
		else if (s.k === 'set' && s.e.k === 'load' && s.e.size === 8 && isS(s.e.addr)) all.push({ b: bi, i, v: s.dst, idx: -1 })
		else if (s.k === 'store' && s.size === 8 && isS(s.addr) && s.v.k === 'bin' && s.v.op === 'add' && s.v.a.k === 'var' && s.v.b.k === 'const' && s.v.b.v % 0x30n === 0n) all.push({ b: bi, i, adv: [s.v.a.id, Number(s.v.b.v / 0x30n)], idx: -1 })
	}))
	all.sort((x, y) => g.rpo[x.b] - g.rpo[y.b] || x.i - y.i)
	if (all.some((x, k) => k && !(all[k - 1].b === x.b ? all[k - 1].i < x.i : dominates(g, all[k - 1].b, x.b)))) return undefined
	let n = 0
	const at = new Map<number, number>()
	for (const x of all) {
		if (x.adv) { const k = at.get(x.adv[0]); if (k === undefined) return undefined; n = k + x.adv[1] }
		else { x.idx = n; if (x.v !== undefined) at.set(x.v, n); else n++ }
	}
	// (a load not followed by an advance past it is taken too)
	if (all.length && all[all.length - 1].v !== undefined) n++
	const evs = all.filter(x => !x.adv)
	if (n !== names.length || evs.some(x => x.idx >= names.length)) return undefined
	// (the variables holding an &AccountInfo taken as is, with the accounts after it in the slice)
	const ptrs = new Map<number, string>(), seqs = new Map<number, string[]>()
	for (const x of evs) if (x.v !== undefined && !TD.multi.has(x.v)) { ptrs.set(x.v, names[x.idx]); seqs.set(x.v, names.slice(x.idx)) }
	// (and those the decompiler names after an account (an AccountInfo the checks name))
	T.f.vars.forEach(v => { const n = T.names[v.id]; if (n && names.includes(n) && !TD.multi.has(v.id) && TD.defs.has(v.id) && !ptrs.has(v.id) && T.text.includes(`const ${n}: AccountInfo = `)) ptrs.set(v.id, n) })
	return { names, T, TD, g, evs, ptrs, seqs, tpc: tpc! }
}

function byValueTry(r: Result, H: FuncOut): TryInfo | undefined {
	const se = sliceEvents(r, H)
	if (!se) return undefined
	const { names, T, TD, evs, ptrs, seqs, tpc } = se, cl = calleeOf(r), out = paramVar(T.f, 1)
	...
	const trace = (z: number, p: number): [number, number] | undefined => {
		for (let n = 0; n < 8; n++) {
			...
			if (e.k === 'call') {
				...
				if (k >= 0) return [evs[k].idx, z - evs[k].out!]
	...
	const ptrEv = (e: Expr, p: number): number | undefined => {
		for (let n = 0; n < 8; n++) {
			if (e.k === 'var') {
				...
				if (k >= 0 && !TD.multi.has(e.id)) return evs[k].idx
	...
	return words.size || ptrs.size ? { tryPc: tpc!, layout: [], words, ptrs, seqs } : undefined
}
	...
	const visit = (C: FuncOut, roots: Map<number, HVal>, depth: number) => {
		...
		const each = (s: Stmt, bi: number, i: number) => {
			...
			if (c?.t.k === 'fn' && /try_borrow_data/.test(r.program.funcs.get(c.t.pc)?.name ?? '') && !ff.checks.some(k => k.kinds.includes('discriminator'))) for (const x of c.args) {
				...
				if (v?.k === 'info' && v.off === 0 && !v.guess) { let m = dataReadsMemo.get(r); if (!m) dataReadsMemo.set(r, (m = new Map())); let st = m.get(H.pc); if (!st) m.set(H.pc, (st = new Set())); st.add(v.acct) }
			}
...
const seedMemo = new WeakMap<object, Map<string, AcctResolver>>()
...
// (memoized per function and per kind of callee: the callees the analysis and the decompiler pass answer alike (the same
// functions and names), none (state.ts) means calls write their first 0x80 bytes: the result never depends on which
// caller asked first)
const defsMemo = new WeakMap<VarFunc, Defs>(), defsMemoNc = new WeakMap<VarFunc, Defs>()
export function defsOf(f: VarFunc, callee?: Callee): Defs {
	const DM = callee ? defsMemo : defsMemoNc
	let d0 = DM.get(f)
	if (d0) return d0
...
export function accountResolver(fo: { f: VarFunc; names: string[] }, callee?: Callee, seed?: Map<number, AcctVal>): AcctResolver {
	...
	const sk = `${ck}${seed?.size ? JSON.stringify([...seed]) : ''}`
	let res = seedMemo.get(fo.f)?.get(sk)
	if (res) return res
	const f = fo.f
